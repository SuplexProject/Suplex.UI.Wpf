...
{
	public partial class FileSaveSignedDlg : Window
	{
		...
		public FileSaveSignedDlg()
		{
			InitializeComponent();

			Application.Current.MainWindow.Closing +=
				new System.ComponentModel.CancelEventHandler( MainDlg_Closing );
		}
		...
		{
			BrowseSaveFileAs( txtSaveFile, FilterOption.Suplex );
		}
		...
		{
			BrowseSaveFileAs( txtPubPrivKeys, FilterOption.PublicPrivateKeyPair );
		}

		private void cmdBrowsePubOnly_Click(object sender, RoutedEventArgs e)
		{
			BrowseSaveFileAs( txtPubOnlyKey, FilterOption.PublicOnlyKey );
		}

		private void cmdGenerateKeys_Click(object sender, RoutedEventArgs e)
		{
			if( !string.IsNullOrEmpty( PublicPrivateContainerName ) )
			{
				CspParameters cspParams = new CspParameters();
				cspParams.KeyContainerName = PublicPrivateContainerName;
				RSACryptoServiceProvider rsaKey = new RSACryptoServiceProvider( cspParams );

				if( !string.IsNullOrEmpty( PublicPrivateFileName ) )
				{
					using( StreamWriter sw = new StreamWriter( PublicPrivateFileName ) )
					{
						sw.Write( rsaKey.ToXmlString( true ) );
					}

					if( !HasKeysContainerName )
                        KeysContainerName = PublicPrivateContainerName;

                    if( !HasKeysFileName )
                        KeysFileName = PublicPrivateFileName;
                }

                if( !string.IsNullOrEmpty( PublicOnlyFileName ) )
                    using( StreamWriter sw = new StreamWriter( PublicOnlyFileName ) )
                    {
